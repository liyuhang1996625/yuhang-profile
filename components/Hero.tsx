
import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { ArrowDown } from 'lucide-react';
import { Language, Theme } from '../types';
import HeroBackground from './HeroBackground';

interface HeroProps {
  lang: Language;
  theme: Theme;
}

const Hero: React.FC<HeroProps> = ({ lang, theme }) => {
  const [time, setTime] = useState(new Date());

  useEffect(() => {
    const timer = setInterval(() => {
      setTime(new Date());
    }, 1000);
    return () => clearInterval(timer);
  }, []);

  return (
    <section id="about" className="min-h-screen flex flex-col justify-center px-6 md:pl-32 md:pr-12 pt-20 relative border-b border-tech-200 dark:border-tech-900 bg-grid transition-colors duration-300 overflow-hidden">
      
      {/* WebGL Background */}
      <HeroBackground theme={theme} />

      {/* Decorative Technical Elements - Clock (Updated: Removed LOC/LAT) */}
      <div className="absolute top-32 right-12 font-mono text-[10px] text-tech-400 dark:text-tech-600 hidden md:block text-right leading-relaxed select-none z-10">
        TIME: {time.toLocaleTimeString('en-GB', { hour12: false })}<br/>
        DATE: {time.toISOString().split('T')[0]}
      </div>

      <div className="max-w-4xl z-10 pointer-events-none"> {/* Content is pointer-events-none to let mouse pass through to canvas if needed, but here we want interaction everywhere */}
        <div className="pointer-events-auto">
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5 }}
            className="flex items-center gap-3 mb-8"
          >
            <div className="w-2 h-2 bg-green-500 dark:bg-neon rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)] dark:shadow-[0_0_10px_#00FF41]"></div>
            <span className="font-mono text-xs text-tech-500 dark:text-neon uppercase tracking-widest">
              {lang === 'zh' ? '系统就绪' : 'System Ready'}
            </span>
          </motion.div>

          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1, ease: "easeOut" }}
            className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tighter text-black dark:text-white leading-[1.1] mb-8"
          >
            {lang === 'zh' ? 'UI 工程师' : 'UI ENGINEER'} <span className="text-tech-300 dark:text-tech-700 font-normal">/</span><br />
            {lang === 'zh' ? '数字工匠' : 'DIGITAL CRAFTSMAN'}
          </motion.h1>

          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.3 }}
            className="max-w-xl border-l-2 border-tech-200 dark:border-tech-800 pl-6 bg-white/50 dark:bg-black/50 backdrop-blur-sm py-2"
          >
            <p className="font-mono text-xs text-tech-400 dark:text-tech-500 mb-2">{'//'} {lang === 'zh' ? '简介' : 'INTRO'}</p>
            <p className="text-base md:text-lg text-tech-700 dark:text-tech-200 leading-relaxed font-medium">
              {lang === 'zh' 
                ? '将复杂的逻辑转化为极简、实用的界面。我构建的数字产品注重清晰度、精确性和高性能。'
                : 'Translating complex logic into minimal, functional interfaces. I build digital products that prioritize clarity, precision, and performance.'}
            </p>
          </motion.div>
        </div>
      </div>

      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 1, duration: 1 }}
        className="absolute bottom-10 left-6 md:left-32 flex items-center gap-4 z-10"
      >
        <div className="w-px h-12 bg-tech-300 dark:bg-tech-700"></div>
        <div className="flex items-center gap-2 font-mono text-[10px] text-tech-500 dark:text-tech-500 uppercase tracking-widest">
          <ArrowDown className="w-3 h-3 text-tech-400 dark:text-neon" />
          <span>{lang === 'zh' ? '向下滚动' : 'Scroll_Down'}</span>
        </div>
      </motion.div>
    </section>
  );
};

export default Hero;
